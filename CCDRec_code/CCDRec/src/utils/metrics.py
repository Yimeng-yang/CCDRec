...
#     ipdb.set_trace()
    ...
    df = pd.DataFrame(data=rec_ret)
    file_path = "recall_result_light_neg.csv"
    df.to_csv(file_path)
...
def ndcg_(pos_index, pos_len):
    ...
    file_path = "ndcg_result_light_neg.csv"
    df.to_csv(file_path)
...
def map_(pos_index, pos_len):
    ...
    df = pd.DataFrame(data=result)
    file_path = "map_result_light_neg.csv"
    df.to_csv(file_path)
    return result.mean(axis=0)
...
def precision_(pos_index, pos_len):
    ...
    df = pd.DataFrame(data=rec_ret)
    file_path = "p_result_light_neg.csv"
    df.to_csv(file_path)
    return rec_ret.mean(axis=0)
